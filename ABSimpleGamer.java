	...
	private int monteCarloDepthSearch(long timeout) throws TransitionDefinitionException, MoveDefinitionException {  
        ...
        while(true) {
		    ...
		    while(!getStateMachine().isTerminal(state)) {
		    	state = getStateMachine().getNextStateDestructively(state, getStateMachine().getRandomJointMove(state));
		    	int localMax = 0;
		    	for(int i = 0; i < getStateMachine().getRoles().size(); i++){
		    		int temp = getStateMachine().getLegalMoves(state, getStateMachine().getRoles().get(i)).size();
		    		if(temp >= localMax){
		    			localMax = temp;
		    		}
		    	}
